...
def extract_diesel_uf_and_type():
    extractSheet('DPCache_m3','extract_diesel_by_uf_and_type.xlsx','/diretorio/desafio.xls')    

def extract_oil_derivative_fuels_by_uf_and_product():
    extractSheet('DPCache_m3 2','extract_oil_derivative_fuels_by_uf_and_product.xlsx','/diretorio/desafio.xls')
...
with DAG(
    ...
    transformation = DockerOperator(
        ...
        api_version='auto',
        auto_remove=True,
        command='libreoffice --invisible --headless --convert-to xlsx --outdir /tmp /tmp/ "{0}"'.format('desafio.xls'),
    ...
    extractFiles >> transformation
    transformation >> extract_oil_derivative_fuels_by_uf_products
    transformation >> extract_diesel_uf_types
